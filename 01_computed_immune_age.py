...
def predict_age(sample_file, file_name, path):
    ...
    for file in sample_file:
        data_file = os.listdir(path+'Output/%s/%s' % (file_name, file))
        new_data = pd.read_csv(path+'Output/%s/%s' % (file_name, file)+'/'+data_file[0])
...
if __name__ == '__main__':
    ...
    file_name = 'test'
    os.mkdir(path+'Output/%s' % file_name)
    ...
    for info in df.columns[:]:
        # 对每个样本创建一个文件夹
        os.mkdir(path+'Output/%s/%s' % (file_name, info[:-4]))
        # 取出样本数据并生成固定格式的数据框
        ...
        feature_df.to_csv(path+'Output/%s/%s' % (file_name, info[:-4]) + '/NewData_Sample_%s' % info[:-4] + '.csv', index=False)


    ########################
    #### age prediction ####
    ########################
    sample_file = os.listdir(path+'Output/%s' % file_name)
